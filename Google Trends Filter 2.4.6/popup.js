...
let searchCountry = 'US';  // Nouveau: pour la section recherche
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    chrome.storage.local.get([
        'selectedCountry', 
        'searchSelectedCountry',  // Nouveau: stockage du pays de recherche
        'activeMainMenu', 
        'activeSubMenu'
    ], function(result) {
        ...
        if (result.searchSelectedCountry) {  // Nouveau
...
window.addEventListener('unload', function() {
    ...
    chrome.storage.local.set({ 
        ...
        'searchSelectedCountry': searchCountry,  // Nouveau
...
function setupTrendSearch() {
    ...
    timeRange.addEventListener('change', performTrendSearch);
    countrySelect.addEventListener('change', changeSearchCountry);
...
function updateUILanguage() {
    ...
    // Nouvelles mises à jour pour la section recherche
    ...
    timeRangeSelect.options[4].text = t['lastMonth'];
    timeRangeSelect.options[5].text = t['last3Months'];
    timeRangeSelect.options[6].text = t['lastYear'];
...
function updateNewsSectionText() {
    ...
    if (summaryElement.textContent.includes('Loading news...') || 
        summaryElement.textContent.includes('Chargement des actualités...')) {
        summaryElement.textContent = translations[currentLanguage]['loadingNews'];
               ...
               summaryElement.textContent.includes('Veuillez entrer un mot-clé')) {
...
function updateTrendsSectionText() {
    ...
    if (trendsList.textContent.includes('Loading trends...') || 
        trendsList.textContent.includes('Chargement des tendances...')) {
        trendsList.innerHTML = `<tr><td colspan="4">${translations[currentLanguage]['loadingTrends']}</td></tr>`;
               ...
               trendsList.textContent.includes('Impossible de récupérer les tendances')) {
