...
const API_KEY = 'API'; // Remplacez par votre clé API Perplexity (laissez vide si vous n'en avez pas)
...
// ==========================================
// Fonctions responsive pour les iframes
// ==========================================
function setupResponsiveIframes() {
    // Observer pour détecter les changements de taille du sidepanel
    let resizeTimeout;
    ...
    // Observer le body ou le conteneur principal
...
function adjustIframeHeights() {
    const iframes = document.querySelectorAll('.trends-iframe');
    const sidepanelWidth = document.body.clientWidth;
    
    let newHeight;
    let geoHeight; // Special height for geographic distribution
    
    if (sidepanelWidth >= 800) {
        newHeight = '600px';
        geoHeight = '700px'; // More space for geographic distribution
    } else if (sidepanelWidth >= 600) {
        newHeight = '550px';
        geoHeight = '650px';
    } else if (sidepanelWidth >= 400) {
        newHeight = '500px';
        geoHeight = '600px';
    } else {
        newHeight = '450px';
    ...
    iframes.forEach(iframe => {
        ...
        // Check if this is the geographic distribution iframe
        if (iframe.id === 'compareGeoFrame') {
            iframe.style.height = geoHeight;
            // Only refresh if the height actually changed and iframe has content
            if (oldHeight !== geoHeight && iframe.src && iframe.src.trim() !== '') {
                setTimeout(() => {
                    refreshIframeContent(iframe);
                }, 100); // Small delay to let the height change take effect
            }
        } else {
            iframe.style.height = newHeight;
            // Refresh other iframes if height changed significantly
            if (oldHeight !== newHeight && iframe.src && iframe.src.trim() !== '') {
                const oldHeightNum = parseInt(oldHeight) || 0;
                const newHeightNum = parseInt(newHeight) || 0;
                if (Math.abs(oldHeightNum - newHeightNum) > 50) { // Only refresh if height change is significant
                    setTimeout(() => {
                        refreshIframeContent(iframe);
                    }, 100);
                }
...
function refreshIframeContent(iframe) {
    ...
    if (currentSrc && currentSrc.trim() !== '') {
        // Remove any existing refresh parameter and add a new one
        const separator = currentSrc.includes('?') ? '&' : '?';
        const newSrc = currentSrc.split('&_refresh=')[0].split('?_refresh=')[0] + separator + '_refresh=' + Date.now();
        iframe.src = newSrc;
        console.log('Refreshed iframe:', iframe.id, 'with new height:', iframe.style.height);
    }
}
