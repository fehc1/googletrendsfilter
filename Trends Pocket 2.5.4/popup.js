...
const API_KEY = 'pplx-API'; // Remplacez par votre clé API Perplexity
...
// ==========================================
// Fonctions de nettoyage et cache
// ==========================================
async function cleanExpiredCache() {
    try {
        ...
        for (let [key, value] of Object.entries(storage)) {
            if (key.startsWith('trend_explanation_')) {
        ...
        if (toRemove.length > 0) {
            ...
            console.log(`Cleared ${toRemove.length} expired cache entries`);
        }
    } catch (error) {
...
// ==========================================
// Variables et fonctions pour les explications de tendance
// ==========================================
let trendExplanationsCache = new Map();

async function getPerplexityExplanation(trend) {
    // Ajouter la langue à la clé du cache
    ...
    try {
        const systemPrompt = currentLanguage === 'fr' 
            ? "Tu es un assistant spécialisé dans l'actualité. Tu dois fournir des résumés très courts en 1-2 phrases maximum. Réponds en français."
            : "You are a news specialist assistant. You must provide very short summaries in 1-2 sentences maximum. Answer in English.";
        
        const userPrompt = currentLanguage === 'fr'
            ? `Explique très brièvement pourquoi "${trend}" est une tendance actuellement ? Donne uniquement l'information essentielle en 1-2 phrases.`
            : `Briefly explain why "${trend}" is trending right now? Give only the essential information in 1-2 sentences.`;
        ...
        // Mettre en cache et retourner la réponse
        ...
        return currentLanguage === 'fr'
            ? `Impossible de charger l'explication pour "${trend}"`
            : `Unable to load explanation for "${trend}"`;
    }
}


let hideTimeout; // Variable globale pour gérer le timeout
...
function showTrendModal(event, trend) {
    ...
    // Get trend table boundaries
    const trendTable = document.querySelector('.trends-table');
    const tableBounds = trendTable.getBoundingClientRect();
    
    // Calculate position relative to table
    let x = event.clientX - tableBounds.left + 10;
    let y = event.clientY - tableBounds.top - 10;
    
    // Keep modal within table bounds
    const modalWidth = 250;
    if (x + modalWidth > tableBounds.width) {
        x = x - modalWidth - 20;
    }
    ...
    getPerplexityExplanation(trend).then(explanation => {
        ...
        modalText.style.display = 'block';
        console.error('Error loading trend explanation:', error);
    });
}
...
function displayEmergingTrends(text) {
    ...
    document.querySelectorAll('.trend-link').forEach(link => {
        ...
        link.addEventListener('mouseleave', (e) => {
            // Vérifie si la souris n'est pas sur la modal
            const modal = document.getElementById('trendModal');
