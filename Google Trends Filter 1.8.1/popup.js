...
function applyFilter() {
  ...
  const explorationDepth = parseInt(document.getElementById('explorationDepth').value);
  ...
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    chrome.tabs.sendMessage(tabs[0].id, {action: "filter", keyword: currentKeyword, explorationDepth: explorationDepth})
      .then(response => {
        clearInterval(loadingInterval);
        isFiltering = false;
        console.log("Réponse reçue:", response);
        if (response.success) {
          const resultText = `<i class="material-icons" style="color: green;">check_circle</i> Filtrage appliqué pour "${currentKeyword}". ${response.count} résultat(s) trouvé(s) sur ${response.totalCount} éléments explorés.`;
