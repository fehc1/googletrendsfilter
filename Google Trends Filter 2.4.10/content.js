...
function detectGoogleSearch() {
    ...
    // Si c'est la même requête qu'avant, on ne fait rien
...
function updateTrendingPanel(keyword) {
    ...
    iframe.onload = () => {
        setTimeout(() => {
            ...
            // Configurer l'auto-hide après 4.5 secondes
            autoHideTimeout = setTimeout(() => {
                // Cacher le panneau en douceur seulement si la souris n'est pas dessus
                if (!mouseOverPanel) {
                    if (panel) {
