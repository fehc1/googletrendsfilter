...
// Fonction pour encoder les caractères spéciaux
...
async function filterAndDisplayResults(keyword) {
  console.log(`Début du filtrage. Mot-clé: "${keyword}"`);
  
  const trendElements = document.querySelectorAll('[data-row-id], .mZ3RIc[jsname="oKdM2c"]');
  console.log(`Nombre d'éléments de tendance trouvés: ${trendElements.length}`);

  if (trendElements.length === 0) {
    console.error("Aucun élément de tendance trouvé. Les sélecteurs CSS peuvent être obsolètes.");
    return { success: false, count: 0, totalExplored: 0, error: "Aucun élément trouvé" };
  }
  ...
  for (const element of trendElements) {
    ...
    console.log(`Élément: "${trendText.substring(0, 50)}..."`);
    console.log(`  Correspond au mot-clé: ${matchesKeyword}`);
  ...
  console.log(`Filtrage terminé. ${matchCount} correspondances trouvées sur ${trendElements.length} éléments.`);
  return { success: true, count: matchCount, totalExplored: trendElements.length };
}

// Fonction pour créer un CSV
...
// Fonction pour obtenir les requêtes associées
...
async function exportData(type, keyword) {
  console.log("Exportation des données commencée pour:", type, keyword);
  ...
  let csvData;
  if (type === 'requetes') {
    csvData = createCSV([['Sujet', 'Requêtes associées'], ...filteredData]);
  } else if (type === 'ngrams') {
    // Implémentez ici la logique pour les n-grams si nécessaire
  ...
  console.log("Exportation des données terminée.");
  return { success: true };
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === "filter") {
    filterAndDisplayResults(request.keyword)
      .then(result => {
        sendResponse(result);
        // Envoyer les statistiques de filtrage à la popup
        chrome.runtime.sendMessage({
          action: "filterStats",
          count: result.count,
