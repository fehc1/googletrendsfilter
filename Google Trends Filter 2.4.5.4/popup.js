...
let currentLanguage = 'en'; // Changed from 'fr' to 'en'
let newsCache = {};
let lastKeyword = '';
let currentCountry = 'US'; // Changed from 'FR' to 'US' to match default language
...
function updateExistingTrendsTranslation() {
    ...
    document.querySelectorAll('.trend-start').forEach(element => {
        ...
        } else if (dateText.includes('heure') || dateText.includes('hour')) {
            unit = number === 1 ? 'hour' : 'hours';
        } else if (dateText.includes('jour') || dateText.includes('day')) {
...
function displayEmergingTrends(text) {
    ...
    // Tri initial par date de publication (du plus récent au plus ancien)
    ...
    trendsData.forEach(trend => {
        trendsHTML += `
            <tr>
                <td class="trend-title">${trend.title}</td>
